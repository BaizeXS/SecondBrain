...
import { useAgents, getIconComponent } from '../../contexts/AgentContext'; // 确保路径正确
import {
...
// --- 简化数据的辅助函数，用于回调 ---
const simplifyFileForCallback = f => ({ id: f.id, name: f.name, size: f.size, type: f.type, preview: f.preview, uploadedAt: f.uploadedAt });
const simplifyNoteForCallback = n => ({ id: n.id, name: n.name, preview: n.preview, content: n.content, createdAt: n.createdAt });
      ...
      {displayedItems.length > 0 ? (
        <ul className={styles.itemList}>
          {displayedItems.map(file => {
            ...
            return (
              <li key={file.id} className={styles.itemCard} onContextMenu={(e) => handleFileContextMenu(e, file.id)}>
                ...
                <div className={styles.itemInfo}>
                  ...
                  {currentProjectId ? (
                    ...
                    <span className={styles.itemName}>{file.name}</span> // 如果不在项目上下文中，则不可点击
                  )}
                  <p className={styles.itemPreview}>{file.preview || formatFileSize(file.size || 0)}</p>
                </div>
              </li>
            );
          })}
        </ul>
      ) : (<p className={styles.emptyStateText}>{searchQuery || Object.values(activeFilters).some(arr => arr?.length > 0) ? `No files match your criteria.` : 'No files to display. Click "+" to add.'}</p>)}
      {contextMenuVisible && <ContextMenu menuItems={contextMenuItems} position={contextMenuPosition} onClose={closeContextMenu} targetId={contextMenuTargetId} />}
      ...
      {displayedItems.length > 0 ? (
        <ul className={styles.itemList}>
          {displayedItems.map(note => (
            <li key={note.id} className={styles.itemCard} title="Click to edit note">
              <FiEdit3 className={styles.itemTypeIcon} />
              <div className={styles.itemInfo} onClick={() => handleEditNoteClick(note)} style={{ cursor: 'pointer', flexGrow: 1 }}>
                <span className={styles.itemName}>{note.name}</span>
...
const FileSpecificChatView = ({ data }) => {
  ...
  const handleSendMessage = (messageText, filesFromInput) => {
    ...
    updateProject(data.projectId, { fileChats: updatedFileChats, files: updatedProjectFiles });
  };
            ...
            <div key={msg.id || msg.timestamp} className={`${styles.sidebarChatMessage} ${styles[msg.sender]}`}>
              ...
              <div className={styles.messageBubbleContent}>
                ...
                {msg.files && msg.files.length > 0 && (
                  <MessageFileAttachments files={msg.files} isAiMessage={msg.sender === 'ai'} />
                )}
              </div>
            </div>
          ))
