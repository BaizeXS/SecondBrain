...
const ProjectChatView = ({ projectData, activeSessionId, onSendMessageToProject }) => {
  ...
  const handleSendMessageViaInterface = (messageText, filesFromInput) => {
    // 传递整个 activeAgentObject，以便上层可以访问所有API配置
    onSendMessageToProject(messageText, filesFromInput, activeSessionId, projectChatActiveAgentId);
  };
...
const ProjectPage = () => {
  ...
  const { projects, getProjectById, updateProject, loadingProjects: projectsContextLoading } = useProjects();
  ...
  const handleSendMessageToProject = async (messageText, attachedFiles, currentActiveSessionId, agentId) => {
    ...
    const newUserMessage = {
      ...
      files: attachedFiles.map(f => ({ id: f.id, name: f.name, size: f.size, type: f.type, uploadedAt: f.uploadedAt, preview: f.preview })),
    ...
    if (targetSessionId) {
      ...
      if (sessionIndex !== -1) {
        ...
        const newSessionFiles = attachedFiles.map(f => ({ id: f.id, name: f.name, type: f.type, size: f.size, uploadedAt: f.uploadedAt }));
    ...
    if (!targetSessionId) {
      ...
      const newSession = {
        ...
        sessionFiles: attachedFiles.map(f => ({ id: f.id, name: f.name, type: f.type, size: f.size, uploadedAt: f.uploadedAt })),
    ...
    if (sessionModifiedOrCreated) {
      ...
      // 只有在会话被修改或有新文件添加到项目时才更新
      if (sessionModifiedOrCreated || filesWereAddedToProject) {
    ...
    <div className={styles.projectPage}>
      <div className={styles.mainContentArea}>
        <div className={styles.projectHeader}>
          ...
          {/* 移除了右侧边栏切换按钮，由 RightSidebar 自身拉手控制，或由特定页面逻辑通过 context 控制 */}
          {/* <button onClick={handleToggleProjectRightSidebar} className={styles.sidebarToggleButtonOnPage}> {isProjectSidebarOpen ? <FiChevronsRight /> : <FiChevronsLeft />} </button> */}
        </div>
