...
class SpaceService:
    ...
    async def create_space(
        ...
        """创建新空间."""
        # 检查是否已存在同名空间
        existing = await crud.crud_space.get_by_name(
        ...
        # 创建空间
        return await crud.crud_space.create(db, obj_in=space_data, user_id=user.id)
    ...
    async def get_user_spaces(
        ...
        """获取用户的空间列表."""
        return await crud.crud_space.get_user_spaces(
    ...
    async def update_space(
        ...
        """更新空间信息."""
        return await crud.crud_space.update(db, db_obj=space, obj_in=space_data)

    @staticmethod
    async def delete_space(db: AsyncSession, space: Space) -> bool:
        """删除空间."""
        await crud.crud_space.remove(db, id=space.id)
        return True
    ...
    async def count_user_spaces(db: AsyncSession, user: User) -> int:
        ...
        return await crud.crud_space.get_count(db, query=query)
    ...
    async def update_space_stats(
        ...
        """更新空间统计信息."""
        return await crud.crud_space.update_stats(
            db,
            space_id=space_id,
